...
- (instancetype)initWithCoder:(NSCoder *)aDecoder
{
    ...
    if (self) {
        self.view = [[[NSBundle mainBundle] loadNibNamed:@"HQVideoPlayer" owner:self options:nil] firstObject];
...
- (void)specialTitle:(NSString *)mediaTitle {
...
//                NSLog(@"%f", CMTimeGetSeconds(duration));
        ...
        // 下面这部分代码抽空可以优化一下，使得缓冲进度条功能更加齐备      --heqin
//        NSTimeInterval timeInterval = [self availableDurationRanges];//缓冲进度
//        CMTime duration = _playerItem.duration;
//        CGFloat totalDuration = CMTimeGetSeconds(duration);
        
        // 由于在播放完成后，使当前的状态为暂停状态时，bufferProgress会重新加载，
        // 用户看到的现象会是bufferProgress会重新加载，线条会退回当前的加载位置。比较奇怪，所以这里暂时不进行缓冲进度的显示。
        // 但是不把bufferProgress进行隐藏，放在这里用来做为显示加载进度的进度条。所以把下面一行进行屏掉
...
- (void)setMaxDuratuin:(float)total{
    ...
    self.totalTimeLabel.text = [NSString hqConvertTime:self.playProgress.maximumValue];
}
...
- (void)updateVideoSlider:(float)currentTime{
    ...
    self.currentTimeLabel.text = [NSString hqConvertTime:currentTime];
    self.speedLabel.text = [NSString stringWithFormat:@"%@/%@", [NSString hqConvertTime:currentTime], [NSString hqConvertTime:self.playProgress.maximumValue]];
}
...
- (IBAction)rotatingAction:(id)sender {
    ...
    if (self.isLockScreen) {
        ...
        if([HQRotatingScreen isOrientationLandscape]) {
            ...
            [HQRotatingScreen forceOrientation: UIInterfaceOrientationLandscapeRight];
